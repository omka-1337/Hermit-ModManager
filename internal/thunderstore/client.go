...
func (c *Client) fetchURL(ctx context.Context, fullURL string) ([]byte, error) {
	body, err := c.getURL(ctx, fullURL)
	if err != nil {
		return nil, err
	}
	defer body.Close()
...
func (c *Client) getURL(ctx context.Context, u string) (io.ReadCloser, error) {
